...
func TestBuildExecutionPlanRejectsUnsafeTestNames(t *testing.T) {
	...
	_, err := buildExecutionPlan(map[packageKey]*packageSelection{selection.Key: selection}, planOptions{})
...
func TestBuildExecutionPlanKeepsManyExactTestsPrecise(t *testing.T) {
	...
	result, err := buildExecutionPlan(map[packageKey]*packageSelection{selection.Key: selection}, planOptions{})
...
func TestBuildExecutionPlanCoalesceMergesPackagesAndUnionsTests(t *testing.T) {
	t.Parallel()
	...
	selections := map[packageKey]*packageSelection{
		...
		{Dir: "pkgtwo", Name: "two"}: {
			...
			Tests: map[string]struct{}{"TestGamma": {}},
			Files: map[string]struct{}{"pkgtwo/two_test.go": {}},
		},
	}
	result, err := buildExecutionPlan(selections, planOptions{Coalesce: true})
	require.NoError(t, err)
	require.Len(t, result.Matrix.Include, 1)
	entry := result.Matrix.Include[0]
	require.Equal(t, "./pkgone ./pkgtwo", entry.Package)
	require.Equal(t, "^(TestAlpha|TestBeta|TestGamma)(/.*)?$", entry.RunRegex)
	require.Equal(t, "10", entry.TestCount)
	// Summary stays per-package so humans can still see the provenance.
	require.Len(t, result.Summary.Entries, 2)
	require.Equal(t, "./pkgone", result.Summary.Entries[0].Label)
	require.Equal(t, "./pkgtwo", result.Summary.Entries[1].Label)
}

func TestBuildExecutionPlanCoalesceDeduplicatesTestNames(t *testing.T) {
	t.Parallel()
	...
	selections := map[packageKey]*packageSelection{
		...
		{Dir: "pkgtwo", Name: "two"}: {
			Key:   packageKey{Dir: "pkgtwo", Name: "two"},
			Tests: map[string]struct{}{"TestShared": {}, "TestExtra": {}},
			Files: map[string]struct{}{"pkgtwo/two_test.go": {}},
		},
	}
	result, err := buildExecutionPlan(selections, planOptions{Coalesce: true})
	require.NoError(t, err)
	require.Len(t, result.Matrix.Include, 1)
	require.Equal(t, "^(TestExtra|TestShared)(/.*)?$", result.Matrix.Include[0].RunRegex)
}

func TestBuildExecutionPlanCoalesceWithSinglePackageEmitsSingleRow(t *testing.T) {
	t.Parallel()

	selection := &packageSelection{
		Key:   packageKey{Dir: "pkg", Name: "sample"},
		Tests: map[string]struct{}{"TestOnly": {}},
		Files: map[string]struct{}{"pkg/sample_test.go": {}},
	}
	result, err := buildExecutionPlan(map[packageKey]*packageSelection{selection.Key: selection}, planOptions{Coalesce: true})
	require.NoError(t, err)
	require.Len(t, result.Matrix.Include, 1)
	require.Equal(t, "./pkg", result.Matrix.Include[0].Package)
	require.Equal(t, "^(TestOnly)(/.*)?$", result.Matrix.Include[0].RunRegex)
}

func TestBuildExecutionPlanCoalesceWithNoSelectionsEmitsEmptyMatrix(t *testing.T) {
	t.Parallel()

	result, err := buildExecutionPlan(map[packageKey]*packageSelection{}, planOptions{Coalesce: true})
	require.NoError(t, err)
